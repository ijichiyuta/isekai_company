...
void main() {
  ...
  test('UnlockSummary is derived from balance, not hardcoded (AC-16)', () {
    ...
    final tiny = Balance.fromJsonMaps(
      economyJson: {
        'schema_version': 1, 'start_funds': 100, 'lifespan_weeks': 10,
        'bankruptcy_grace_weeks': 4, 'invention_cash_mult_x100': 2500,
        'invention_fame_mult_x100': 250, 'fame_per_sales_g': 100, 'wage_lv1': 8,
        'hire_cost': 20, 'artisan_output_per_week': 3, 'base_capacity_per_week': 2,
        'base_demand_x100': 300, 'demand_per_fame_x100': 4,
        'rank_up_fame_bonus': 50, 'max_employees': 30,
      },
      materialsJson: {'schema_version': 1, 'materials': [{'id': 0, 'name': 'w', 'cost': 2}]},
      ...
      ranksJson: {'schema_version': 1, 'ranks': [
        {'id': 0, 'name': 'r', 'min_assets': 0, 'min_fame': 0, 'min_recipes': 0,
            'min_employees': 0, 'weekly_fixed_cost': 0, 'tax_bp': 0, 'enabled': true}]},
      unlocksJson: {'schema_version': 1, 'unlocks': [
        {'id': 0, 'key': 'a', 'name': 'A', 'desc': '', 'cost': 100, 'tier': 'free',
            'requires': [], 'mod_type': 'start_funds', 'mod_value': 1, 'infinite': false},
        {'id': 1, 'key': 'b', 'name': 'B', 'desc': '', 'cost': 100, 'tier': 'full',
            'requires': [], 'mod_type': 'start_funds', 'mod_value': 1, 'infinite': false}]},
    );
  ...
  test('a failed purchase leaves the player un-entitled', () async {
    final g = GameController(
        balance: balance, clock: FakeTickClock(), seed: 1, iap: _FakeIap(false));
  ...
  test('entitlements persist in a separate file that survives a balance change',
      () async {
    final tmp = Directory.systemTemp.createTempSync('isekai_ent');
    addTearDown(() => tmp.deleteSync(recursive: true));
    // Save the purchase under the real balance.
    await SaveStore(balance, tmp).saveEntitlements(Entitlements(isFull: true));
