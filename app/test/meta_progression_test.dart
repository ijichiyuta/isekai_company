...
void main() {
  ...
  test('rebirth grants the auto unlock → 2周目 starts at a higher rank (§8.4 #3)',
      () {
    ...
    expect(g.meta.isUnlocked(auto.id), isTrue); // granted on completing a life
    expect(g.state.rank, greaterThan(0)); // 2周目 starts above 行商人 (C-6短縮)
  });

  test('purchaseUnlock spends soul points and advantages the next life (C-6)',
      () {
    final g = GameController(balance: balance, clock: FakeTickClock(), seed: 1);
