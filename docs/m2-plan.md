...
**版数**: v1（2026-08-26、並列計画立案フェーズ）→ **監査で是正あり。`docs/m2-plan-audit.md` の是正事項が優先する**
**方式**: 計画 → 監査 → 実装 → 再監査（ユーザー指定のゲート方式）

> ⚠️ **監査結果（2026-08-26）**: B=NO-GO（実レシピ表・素材cost欠落、需要プールid順配分でengine修正要）、A/C=条件付きGO。
> AC-05は報酬源不足で不成立（イベントが必須）、AC-08/10はeconomy調整だけでは未達。
> **統合順序を組み替え**（需要プール修正→計測基盤→B→A→C校正）、hard/soft gateを再分類。
> 詳細と確定した是正事項は **`docs/m2-plan-audit.md`** を参照（この文書より優先）。
